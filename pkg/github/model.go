...
func (p RepositoryList) Swap(i, j int) { p[i], p[j] = p[j], p[i] }
func (p RepositoryList) Len() int      { return len(p) }
func (p RepositoryList) Less(i, j int) bool {
    if p[i].Owner.Login != p[j].Owner.Login {
        return p[i].Owner.Login < p[j].Owner.Login
    }
    return p[i].Name < p[j].Name
}

type RepositoryList []RepoResponse
